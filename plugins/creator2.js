...
if (command == 'whmods3') {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `yuzu.officialdesu@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://www.youtube.com/channel/`, `👤 Afa luwh 🤓`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://github.com/RaffGremory/`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fakes)
